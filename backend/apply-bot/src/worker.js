...
async function runTask(applyTaskId, ctx) {
  ...
  //
  // SHADOW mode does NOT pause — same bug shape as F2/F4-F10's mode-check fixes
  // (see MEMORY.md), carried forward here since this branch predates F7's own
  // session fix. With `concurrency: 1`, a paused task blocks the ENTIRE queue for
  // up to PAUSE_TIMEOUT_MS (10 minutes), and CAPTCHA is confirmed present on
  // ~100% of real Lever/Greenhouse postings — shadow mode never submits, so
  // there's nothing a human solving the challenge would unblock. Live mode's
  // pause is unchanged and correct.
  ...
  try {
    ...
    // check to pick the right submit-button pattern) works correctly. Harmless for
    // the other adapters' synchronous locateSubmit: a plain Playwright Locator is
    // not thenable, so awaiting one just returns it unchanged.
