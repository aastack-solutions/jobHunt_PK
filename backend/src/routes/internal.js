...
router.post('/apply-bot/tasks/:id/callback', requireApplyBotSecret, async (req, res) => {
  ...
  // carried across F2/F7/F8/F10 — see MEMORY.md): the comment above already said
  // "live mode only" but the code never actually checked task.mode. Under the
  // current worker.js, shadow mode never reports status: 'submitted' (only
