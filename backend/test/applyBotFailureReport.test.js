...
test('unknown_outcome tasks are tracked, not silently dropped from the resolved breakdown', { skip: skipReason }, async () => {
  // See MEMORY.md's F9 entry: 'unknown_outcome' (set for real by applyBotSweep.js
  // when the apply-bot process crashes mid-task) used to count toward `resolved`
  // without appearing in succeeded/failed/abstained anywhere.
  const startedAt = new Date();
  const user = await makeUser();
