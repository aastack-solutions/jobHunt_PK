...
test('resolveNavigationUrl: rewrites an employer-hosted board to the slug-free embed form', () => {
  ...
  for (const url of EMPLOYER_HOSTED_GREENHOUSE) {
    ...
    // which is exactly "hostname contains greenhouse.io" - this is the cross-service
    // contract that makes the whole rewrite worth doing.
    assert.ok(new URL(nav).hostname.includes('greenhouse.io'));
  }
});
