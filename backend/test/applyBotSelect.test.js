...
// time — confirmed directly by running it in isolation. Fixed with a bounded,
// non-retrying PROBE client (2s connectTimeout, reconnectStrategy: false) that
// never touches the shared singleton — only once that probe succeeds does this
// file require applyBotSelect.js/applyBotQueue at all, so the indefinite-retry
// singleton is never given a reason to start retrying in the first place when
...
// isn't available, so every one of the tests below gets this for free just by
// passing `t` through.
