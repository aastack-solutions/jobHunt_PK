...
export default function Signup() {
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      if (status === 409) setFormError('Email already registered.');
      else setFormError(apiMsg || 'Could not create your account. Please try again.');
    }
  }
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="grid w-full max-w-4xl overflow-hidden rounded-[2rem] md:grid-cols-2 glass-strong animate-slide-up">
        ...
        <div className="relative hidden flex-col justify-between bg-gradient-to-br from-brand-blue via-brand-violet to-brand-pink p-10 text-white md:flex">
          ...
          <div>
            <h2 className="text-3xl font-extrabold leading-tight">Join your team's job hunt.</h2>
        ...
        <div className="flex flex-col justify-center bg-white/40 p-8 sm:p-10">
          ...
          <p className="mt-1 text-sm text-slate-500">Start tracking jobs in minutes</p>

          <form onSubmit={handleSubmit(onSubmit)} className="mt-7 flex flex-col gap-4">
            <Input label="Full name" name="fullName" placeholder="Ahmed Khan"
          ...
          <p className="mt-6 text-center text-sm text-slate-500">
            ...
            <Link to={ROUTES.LOGIN} className="font-semibold text-brand-blue hover:underline">
              Sign in
            </Link>
          </p>
        </div>
